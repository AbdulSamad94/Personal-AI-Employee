...
This project's actual agent runtime is [OpenCode](https://opencode.ai), which reads `AGENTS.md` instead of this file (and ignores `CLAUDE.md` entirely if `AGENTS.md` is present — see OpenCode's docs on rules precedence). `AGENTS.md` and this file are kept in sync intentionally, the same way `.agents/skills/` and `.claude/skills/` are duplicated so each tool finds what it needs natively. If you update one, update the other.
...
- **Reasoning**: an LLM agent runs via the [OpenCode](https://opencode.ai) CLI, reads the vault, uses Agent Skills in `.agents/skills/` (OpenCode discovers these natively, no migration needed), and drafts actions. Configured in `opencode.json` at the repo root, currently pointed at OpenCode Zen's free `opencode/big-pickle` model.
...
- MCP servers are registered in `opencode.json`'s `mcp` key (not a separate `config/mcp.json` — that file was removed when the project switched from Qwen Code to OpenCode). Only `mcp_servers/email/server.py` is currently registered there. `odoo_mcp.py` and `socials_mcp.py` exist but aren't wired into `opencode.json` — they aren't reachable by the agent as currently configured.
